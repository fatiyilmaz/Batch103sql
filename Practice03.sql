/*
      CHILD TABLO'DA ON DELETE CASCADE KOMUTU YAZILMAZSA;
   ...
   2: Child tablodaki veri silinmeden Parent tablodaki veri silinmeye calisindiginda PgAdmin Eror verir.
      yani child tablodaki veri silinmeden Parent tablodaki veri silenemez.
   
      CHILD TABLO'DA ON DELETE CASCADE KOMUTU YAZILIRSA;
   
   1: Child tablo silinmeden Parent tablo silinmeye calisildiginda PgAdmin Eror vermez.
      yani Child tablo silinmeden Parent tablo silinebilir.
   2: Child tablodaki veri silinmeden Parent tablodaki veri silinmeye calisildiginda PgAdmin Eror vermez.
      yani Parent tablodaki veriyi siler fakat bu durumda Child tablodaki veri de silinir.
*/
...
update malzemeler set malzeme_isim = (select sirket_ismi from toptancilar where irtibat_ismi = 'Adem Coş') -- select kismi ic sorgu diger kisimlar dis sorgu.
...
update malzemeler set musteri_isim = (select irtibat_ismi from toptancilar where sirket_ismi = 'Apple') -- select kismi ic sorgu diger kisimlar dis sorgu.
...
-- SORU: arac tablosundaki en yüksek fiyat'ı listele Odev
